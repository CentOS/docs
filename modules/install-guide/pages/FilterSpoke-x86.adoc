...
You can install {PRODUCT} on a large variety of storage devices. You can see basic, locally accessible, storage devices in the `Installation Destination` page, as described in xref:StorageSpoke-x86.adoc#sect-disk-partitioning-setup-x86[Installation Destination (x86)]. To add a specialized storage device, click the btn:[Add a disk] button in the `Specialized & Network Disks` section of the screen.
...
* btn:[Add iSCSI Target] - use to attach iSCSI devices; continue with xref:FilterSpoke-x86.adoc#sect-advanced-storage-iscsi-x86[Configure iSCSI Parameters (x86)]

* btn:[Add FCoE SAN] - use to configure a Fibre Channel Over Internet storage device; continue with xref:FilterSpoke-x86.adoc#sect-advanced-storage-fcoe-x86[Configure FCoE Parameters (x86)]
