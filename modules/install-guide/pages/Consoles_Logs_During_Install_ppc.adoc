...
In addition to displaying diagnostic information in [application]*tmux* windows, [application]*Anaconda* also generates several log files, which can be transferred from the installation system. These log files are described in xref:Trouble-ppc.adoc#tabl-installation-log-files-ppc[Log Files Generated During the Installation (ppc)], and directions for transferring them from the installation system are available in xref:Trouble-ppc.adoc#chap-troubleshooting-ppc[Troubleshooting Installation on IBM Power{nbsp}Systems].
