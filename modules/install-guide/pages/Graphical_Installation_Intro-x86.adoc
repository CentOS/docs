...
Your current keyboard layout is displayed in the top right hand corner. Only one layout is configured by default; if you configure more than layout in the `Keyboard Layout` screen (xref:KeyboardSpoke-x86.adoc#sect-keyboard-configuration-x86[Keyboard Configuration (x86)]), you can switch between them by clicking the layout indicator.
