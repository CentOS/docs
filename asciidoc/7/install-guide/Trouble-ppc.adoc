...
In that case, you will have to perform a driver update before starting the installation. Check your hardware vendor's website to determine if a driver update is available that fixes your problem. For more general information on driver updates, see <<chap-driver-updates-ppc>>.
...
See <<sect-recommended-partitioning-scheme-ppc>> for more information.
...
For information about obtaining the most recent installation media, see <<chap-download>>. To perform a media check before the installation starts, append the [option]`rd.live.check` boot option at the boot menu. See <<sect-boot-options-mediacheck>> for details.
