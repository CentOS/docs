...
Register the system::
+
The products installed on a system (including the operating system itself) are covered by subscriptions. A subscription service is used to track registered systems, the products installed on those systems, and the subscriptions attached to those products. Registration is a part of the [application]*Initial Setup* configuration process (see <<sect-initial-setup-subsciption-manager>>).
+
However, if you have not registered your system during [application]*Initial Setup*, you can register it afterwards. See link:++https://access.redhat.com/site/documentation/en-US/Red_Hat_Subscription_Management/1/html/RHSM/index.html++[
Using and Configuring Red{nbsp}Hat Subscription{nbsp}Manager] and link:++https://access.redhat.com/site/documentation/en-US/Red_Hat_Satellite/6.1/html/User_Guide/index.html++[
Red{nbsp}Hat Satellite User Guide] for more information.
+
[NOTE]
====

You can also use the link:++https://access.redhat.com/labs/registrationassistant/++[Registration Assistant] application to guide you through the registration process.
...
+
New software is installed from _package repositories_. Package repositories are organized sets of software and metadata that can be accessed by the [application]*Yum* package manager. If you registered your system with Red{nbsp}Hat, update repositories are configured automatically and you can install updates and additional software from those. However, if you want to set up additional repositories, for example containing your own software, some extra steps are needed.
+
