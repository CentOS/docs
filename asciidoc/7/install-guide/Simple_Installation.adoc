...
+
** If you installed the server using the `Server with GUI` base environment, the [application]*Initial Setup* application is started automatically:
+
... Accept the license agreement.
+
... Register the system.
+
For details, see <<chap-initial-setup>>.
+
...
--username=pass:quotes[_user_name_] --password=pass:quotes[_password_]
....
...
. To register {PRODUCT} after the installation automatically and attach a subscription, append the following to the `/root/rhel-install/anaconda-ks.cfg` file:
+
[literal,subs="+quotes,verbatim,macros"]
....
%post
subscription-manager register --auto-attach --username=pass:quotes[_user_name_] --password=pass:quotes[_password_]
%end
....
...
pass:quotes[`RHEL-7.3\x20Server.x86_64`]
....

. Add a new menu entry to the boot `/root/rhel-install/isolinux/isolinux.cfg` file that uses the Kickstart file. For example:
+
[literal,subs="+quotes,verbatim"]
....
#######################################
