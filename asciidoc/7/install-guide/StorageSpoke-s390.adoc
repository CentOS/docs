...
[[sect-disk-partitioning-setup-s390]]
...
On this screen, you can see storage devices available locally on your computer. You can also add additional specialized or network devices by clicking the btn:[Add a disk] button. To learn more about these devices see <<sect-storage-devices-s390>>.
...
+
If you select the `I will configure partitioning` radio button for manual setup, you will be brought to the `Manual Partitioning` screen after clicking btn:[Done]. See <<sect-custom-partitioning-s390>> for details.
...
When you install {PRODUCT} on a system with both multipath and non-multipath storage devices, the automatic partitioning layout in the installation program might create volume groups that contain a mix of multipath and non-multipath devices. This defeats the purpose of multipath storage.
...
image::images/diskpartsetup/reclaim-space-s390.png[This dialog allows you to preserve, shrink, or delete existing file systems to generate necessary space for the installation.]
