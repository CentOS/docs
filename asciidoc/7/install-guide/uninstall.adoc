...
[[sect-uninstall-rhel-s390]]
=== Removing {PRODUCT} from IBM System{nbsp}z
...
* Make the DASD or SCSI disk where Linux was installed visible from another system, then delete the data. However, this might require special privileges. Ask your system administrator for advice. You can use Linux commands such as [command]`dasdfmt` (DASD only), [command]`parted`, [command]`mke2fs` or [command]`dd`. For more details about the commands, see the respective man pages.

[[sect-uninstall-rhel-s390-another-os]]
==== Running a Different Operating System on Your z/VM Guest or LPAR
