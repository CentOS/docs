...
image::images/pkgselection/pkg-group.png[Example of a Software Selection for a Server Installation]
...
Alternatively, automating the installation with a Kickstart file allows for a much higher degree of control over installed packages. You can specify environments, groups and individual packages in the [command]`%packages` section of the Kickstart file. See <<sect-kickstart-packages>> for instructions on selecting packages to install in a Kickstart file, and <<chap-kickstart-installations>> for general information about automating the installation with Kickstart.
