...
To select the disks and partition the storage space on which you will install Red{nbsp}Hat{nbsp}Enterprise{nbsp}Linux, select `Installation Destination` in the `Installation Summary` screen. If you are unfamiliar with disk partitions, see <<appe-disk-partitions-overview>> for more information.
...
If you have chosen to automatically partition your system, you should manually edit your `/boot` partition; see <<sect-custom-partitioning-x86>> for more details.

* To configure the {PRODUCT} boot loader to _chain load_ from a different boot loader, you must specify the boot drive manually by clicking the `Full disk summary and bootloader` link from the `Installation Destination` screen. See <<sect-bootloader-x86>> for instructions on specifying a boot drive.
...
image::images/diskpartsetup/disk-setup-x86.png[Storage Space Overview.]

On this screen, you can see storage devices available locally on your computer. You can also add additional specialized or network devices by clicking the btn:[Add a disk] button. To learn more about these devices see <<sect-storage-devices-x86>>.
...
+
image::images/diskpartsetup/install-options-reclaim.png[There is not enough room to install {PRODUCT}, so the Installation Options dialog asks if you want to reclaim space from other partitions.]
+
...
+
If you select the `I will configure partitioning` radio button for manual setup, you will be brought to the `Manual Partitioning` screen after clicking btn:[Done]. See <<sect-custom-partitioning-x86>> for details.
...
See <<sect-bootloader-x86>> for more information.
...
image::images/bootloader/x86-bootloader.png[Summary of Selected Disks]
...
image::images/encrypt/passphrase.png[The dialog contains text boxes to enter a passphrase and to confirm it.]
...
image::images/diskpartsetup/reclaim-space-x86.png[This dialog allows you to preserve, shrink, or delete existing file systems to generate necessary space for the installation.]
