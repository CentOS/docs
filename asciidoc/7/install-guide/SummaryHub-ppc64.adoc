...
[[sect-graphical-installation-summary-ppc]]
...
Once all required sections are complete, click the btn:[Begin Installation] button. Also see <<sect-write-changes-to-disk-ppc>>.
...
image::images/preinstallhub/preinstall_no-network_no-wireless.png[Network Configuration Screen When No Network Is Detected]

You can skip this step if you are installing from an installation DVD or other locally accessible media, and you are certain you will not need network to finish the installation. However, network connectivity is necessary for network installations (see <<sect-installation-source-x86>>) or for setting up advanced storage devices (see <<sect-storage-devices-x86>>). For more details about configuring a network in the installation program, see <<sect-network-hostname-configuration-x86>>.