...
An installation of {PRODUCT} requires a minimum of one partition but Red{nbsp}Hat recommends using at least the following partitions or volumes: `/`, `/home`, `/boot`, and `swap`. You can also create additional partitions and volumes as you require. See <<sect-recommended-partitioning-scheme-x86>> for further details.
