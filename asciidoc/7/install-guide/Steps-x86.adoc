...
The UEFI Secure Boot technology requires that the operating system kernel must be signed with a recognized private key in order to be able to boot. In every beta release of {PRODUCT}{nbsp}7, the kernel is signed with a Beta-specific private key, which is different from the more common key used to sign kernels in a General Availability (non-Beta) releases.

The Beta private key will likely not be recognized by your hardware, which means that any Beta release of {PRODUCT} 7 will not be able to boot. In order to use a Beta release with UEFI Secure Boot enabled, you need to add the Beta public key to your system using the _Machine Owner Key_ (MOK) facility.

The procedure to add the Beta key to your system is below.
