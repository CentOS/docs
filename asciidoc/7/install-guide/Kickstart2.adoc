...
In previous versions of {PRODUCT}, Kickstart allowed for upgrading the system as well. In {PRODUCT}{nbsp}7, this functionality has been removed and system upgrades are instead handled by specialized tools. See <<chap-upgrading-your-current-system>> for details.
...
PXE installations are not available on IBM System{nbsp}z.
...
+
Perform the installation in a completely non-interactive command line mode. Any prompt for interaction halts the installation. This mode is useful on IBM System{nbsp}z systems with the x3270 terminal. See <<sect-parameter-configuration-files-kickstart-s390>>.
+
[IMPORTANT]
====
...
+
For the swap sizes assigned by these commands, see <<sect-recommended-partitioning-scheme-x86>> for 64-bit AMD, Intel, and ARM systems, <<sect-recommended-partitioning-scheme-ppc>> for IBM Power{nbsp}Systems servers, and <<sect-recommended-partitioning-scheme-s390>> for IBM System{nbsp}z.
+
...
+
** [option]`--recommended` - Determine the size of the partition automatically. For details about the recommended scheme, see <<sect-recommended-partitioning-scheme-x86>> for 64-bit AMD, Intel, and ARM, <<sect-recommended-partitioning-scheme-ppc>> for IBM Power{nbsp}Systems, and <<sect-recommended-partitioning-scheme-s390>> for IBM System{nbsp}z.
+
[IMPORTANT]
====
...
+
Specify [command]`reboot` to automate installation fully when installing in command line mode on System{nbsp}z.
+
...
One of the most common uses of post-installation scripts in Kickstart installations is automatic registration of the installed system using Red{nbsp}Hat Subscription Manager. The following is an example of automatic subscription in a [command]`%post` script:

[[exam-kickstart-entitlements]]
...
The [command]`subscription-manager` command-line script registers a system to a Red{nbsp}Hat Subscription Management server (Customer Portal Subscription Management, Subscription Asset Manager, or CloudForms System Engine). This script can also be used to assign or attach subscriptions automatically to the system that best-match that system.

When registering to the Customer Portal, use the Red{nbsp}Hat Network login credentials. When registering to Subscription Asset Manager or CloudForms System{nbsp}Engine, use a user account created by the local administrator.
...
Also see the link:++https://access.redhat.com/solutions/748313++[How do I use subscription-manager in a kickstart file?] article on the Red{nbsp}Hat Customer Portal for additional information about using [command]`subscription-manager` in a Kickstart [command]`%post` section.
