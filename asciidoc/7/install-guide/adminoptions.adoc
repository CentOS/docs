...
[option]`inst.sshd=0`::
+
By default, `sshd` is only automatically started on IBM System z, and on other architectures, `sshd` is not started unless the [option]`inst.sshd` option is used. This option prevents `sshd` from starting automatically on IBM System z.

[option]`inst.sshd`::
+
Starts the `sshd` service during the installation, which allows you to connect to the system during the installation using `SSH` and monitor its progress. For more information on SSH, see the `ssh(1)` man page and the corresponding chapter in the link:++https://access.redhat.com/site/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/System_Administrators_Guide/index.html++[Red{nbsp}Hat Enterprise{nbsp}Linux{nbsp}7 System Administrator's Guide]. By default, `sshd` is only automatically started on IBM System z, and on other architectures, `sshd` is not started unless the [option]`inst.sshd` option is used.
+
[NOTE]
====
...
In RHEL 7, network devices are configured and activated during the early stage of installation by the [application]*dracut* modules
...
[option]`upgradeany`::
+
Upgrades are done in a different way in {PRODUCT}{nbsp}7. For more information about upgrading your system, see <<chap-upgrading-your-current-system>>.
