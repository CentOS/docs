...
If you have chosen to automatically partition your system, you should manually edit your `/boot` partition; see <<sect-custom-partitioning-ppc>> for more details.
...
image::images/diskpartsetup/disk-setup-ppc.png[Storage Space Overview.]

On this screen, you can see storage devices available locally on your computer. You can also add additional specialized or network devices by clicking the btn:[Add a disk] button. To learn more about these devices see <<sect-storage-devices-ppc>>.
...
+
image::images/diskpartsetup/install-options-reclaim.png[There is not enough room to install {PRODUCT}, so the Installation Options dialog asks if you want to reclaim space from other file systems.]
+
...
+
If you select the `I will configure partitioning` radio button for manual setup, you will be brought to the `Manual Partitioning` screen after clicking btn:[Done]. See <<sect-custom-partitioning-ppc>> for details.
...
See <<sect-bootloader-ppc>> for more information.
...
image::images/bootloader/ppc-bootloader.png[Summary of Selected Disks]
...
image::images/diskpartsetup/reclaim-space-ppc.png[This dialog allows you to preserve, shrink, or delete existing file systems to generate necessary space for the installation.]
...
include::CustomSpoke-ppc64.adoc[]
