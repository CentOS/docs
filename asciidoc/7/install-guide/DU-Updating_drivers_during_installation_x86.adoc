...
On UEFI systems with the Secure Boot technology enabled, all drivers being loaded must be signed with a valid certificate, otherwise the system will refuse them. All drivers provided by Red{nbsp}Hat are signed by one of Red{nbsp}Hat's private keys and authenticated by the corresponding Red{nbsp}Hat public key in the kernel. If you load any other drivers (ones not provided on the {PRODUCT} installation DVD), you must make sure that they are signed as well.
