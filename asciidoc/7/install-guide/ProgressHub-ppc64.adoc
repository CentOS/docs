...
[[sect-configuration-progress-menu-ppc]]
...
image::images/pkgselection/installing-packages.png[The Packages Completed progress bar]
...
image::images/encrypt/anaconda-entropy.png[A dialog window with a progress bar while the installer gathers entropy]
...
It is possible to disable access to the `Root Password` or `Create User` screens. To do so, use a Kickstart file which includes the [command]`rootpw --lock` or [command]`user --lock` commands. See <<sect-kickstart-commands>> for more information these commands.

include::PasswordSpoke-ppc64.adoc[]

include::UserSpoke-ppc64.adoc[]